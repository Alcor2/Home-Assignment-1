    ...
    def __call__(self, *args):
        ...
        def timer(*args):
            ...
            try:
                ...
                with open("Downloads\EReport.txt", "a") as s:
            ...
            except:     # Writing down the log file's error message
                logging.basicConfig(filename = "Downloads\LogFile.log", format = '%(asctime)s %(message)s', filemode = "w")
                log_file = logging.getLogger()
                log_file.setLevel(logging.ERROR)
                log_file.error(sys.stderr, exc_info = True)
